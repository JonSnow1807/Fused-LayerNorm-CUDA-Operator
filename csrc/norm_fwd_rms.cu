...
//                                                     residual_in)
//
// Dtypes: fp32/fp16/bf16 and fp64 (fp64 exists so torch.autograd.gradcheck
// can exercise the autograd path; it is not a performance target).
// Epilogue: kNone only in this TU - the fp8 epilogues get their own
// instantiation site with the quant kernels.
...
template <typename scalar_t, typename acc_t, bool kFusedAdd>
void launch_rms(const at::Tensor& input2d,
...
void rmsnorm_fwd_cuda_launch(const at::Tensor& input2d,
  ...
  TORCH_CHECK(epi == fused_norm::NormEpilogue::kNone,
              "rmsnorm_fwd_cuda_launch: only the plain epilogue is wired up so far");
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, input2d.scalar_type(), "rmsnorm_cuda", [&] {
        ...
        if (fused_add) {
          launch_rms<scalar_t, acc_t, /*kFusedAdd=*/true>(
              input2d, residual_in2d_or_undef, weight_or_undef, output2d,
              residual_out2d_or_undef, rstd_or_undef, eps, vec, grid, block, stream);
        } else {
          launch_rms<scalar_t, acc_t, /*kFusedAdd=*/false>(
              input2d, residual_in2d_or_undef, weight_or_undef, output2d,
              residual_out2d_or_undef, rstd_or_undef, eps, vec, grid, block, stream);
        }
      });
