...
//
// Dtypes: fp32/fp16/bf16 and fp64 (for gradcheck). Epilogues: kNone,
// kGeluErf/kGeluTanh (train launcher) and, since v0.5.0, kFp8Static/
// kFp8Dynamic (fp32/fp16/bf16 only; inference-only) via
// layernorm_fp8_fwd_cuda_launch, mirroring the RMS family.
...
// Shared kernel-call helper for the fp8 launcher below (the same shape as
// norm_fwd_rms.cu's launch_rms_kernel, with kRMS=false and a bias pointer).
template <typename scalar_t, typename acc_t, bool kFusedAdd, typename Epi>
void launch_ln_kernel(const at::Tensor& input2d,
  ...
  if (vec) {
    fused_norm::norm_fwd_vec_kernel<scalar_t, acc_t, /*kRMS=*/false, kFusedAdd>
        <<<grid, block, 0, stream>>>(x, r, z, y, g, b, /*mean_out=*/nullptr,
                                     /*rstd_out=*/nullptr, N, eps_acc, epi);
  } else {
    fused_norm::norm_fwd_kernel<scalar_t, acc_t, /*kRMS=*/false, kFusedAdd>
        <<<grid, block, 0, stream>>>(x, r, z, y, g, b, /*mean_out=*/nullptr,
                                     /*rstd_out=*/nullptr, N, eps_acc, epi);
  }
...
void layernorm_fp8_fwd_cuda_launch(const at::Tensor& input2d,
  ...
  const int64_t N = input2d.size(1);
  if (M == 0 || N == 0) return;
  TORCH_CHECK(M <= 0x7fffffffLL, "layer_norm_fp8: too many rows for a single launch (", M, ")");
  TORCH_CHECK(epi == NormEpilogue::kFp8Static || epi == NormEpilogue::kFp8Dynamic,
              "layernorm_fp8_fwd_cuda_launch: unsupported epilogue");
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      "layernorm_fp8_cuda", [&] {
        ...
        auto dispatch_fused = [&](auto epi_functor, auto* y_ptr) {
          if (fused_add) {
            launch_ln_kernel<scalar_t, acc_t, /*kFusedAdd=*/true>(
                input2d, residual_in2d_or_undef, weight_or_undef, bias_or_undef, y_ptr,
                residual_out2d_or_undef, eps, epi_functor, vec, grid, block, stream);
          } else {
            launch_ln_kernel<scalar_t, acc_t, /*kFusedAdd=*/false>(
                input2d, residual_in2d_or_undef, weight_or_undef, bias_or_undef, y_ptr,
                residual_out2d_or_undef, eps, epi_functor, vec, grid, block, stream);
          }
        };
