...
> suite (currently 257 tests) is run on hardware before any release. v0.3.0
> (2026‑08‑20) added the first kernel-time measurements; v0.4.0 turned the
> single forward-only kernel into the library described below.
...
clean clone of the release commit. Competitors include the same composite
...
tests/                      257 tests: per-op suites, gradcheck (fp64), bitwise contracts,
                            opcheck, torch.compile fullgraph, determinism
